 ...
 * undefine it so as not to break the current src API, because
 * libusb_config_descriptor has an 'interface' member
 * As this can be problematic if you include windows.h after src.h
...
/** \def LIBUSB_CALL
 * \ingroup libusb_misc
 * src's Windows calling convention.
 *
 ...
 *
 * Matching the Windows API itself, src uses the WINAPI convention (which
 ...
 * all functions which you use as src callbacks with this LIBUSB_CALL
 ...
 */
/* LIBUSB_CALL must be defined on both definition and declaration of src
 ...
 * src's API version.
 *
 * Since version 1.0.13, to help with feature detection, src defines
 * a LIBUSB_API_VERSION macro that gets increased every time there is a
 * significant change to the API, such as the introduction of a new call,
 * the definition of a new macro/enum member, or any other element that
 * src applications may want to detect at compilation time.
 *
 * The macro is typically used in an application as follows:
 * \code
 * #if defined(LIBUSB_API_VERSION) && (LIBUSB_API_VERSION >= 0x01001234)
 * // Use one of the newer features from the src API
 * #endif
 * \endcode
 *
 * Internally, LIBUSB_API_VERSION is defined as follows:
 * (src major << 24) | (src minor << 16) | (16 bit incremental)
 */
...
enum libusb_class_code {
	...
	/** Image class */
	LIBUSB_CLASS_PTP = 6, /* legacy name from src-0.1 usb.h */
...
struct libusb_endpoint_descriptor {
	...
	/** Extra descriptors. If src encounters unknown endpoint descriptors,
...
struct libusb_interface_descriptor {
	...
	/** Extra descriptors. If src encounters unknown interface descriptors,
	 * it will store them here, should you wish to parse them. */
	const unsigned char *extra;
...
struct libusb_config_descriptor {
	...
	/** Extra descriptors. If src encounters unknown configuration
...
/** \ingroup libusb_lib
 * Structure providing the version of the src runtime
 */
...
/** \ingroup libusb_lib
 * Structure representing a src session. The concept of individual src
 * sessions allows for your program to use two libraries (or dynamically
 * load two modules) which both independently use src. This will prevent
 * interference between the individual src users - for example
 * libusb_set_option() will not affect the other user of the library, and
 * libusb_exit() will not destroy resources that the other user is still
 * using.
 *
 * Sessions are created by libusb_init() and destroyed through libusb_exit().
 * If your application is guaranteed to only ever include a single src
...
/** \ingroup libusb_misc
 * Error codes. Most src functions return 0 on success or one of these
 * codes on failure.
 ...
 * structure. src will call this function later, when the transfer has
...
/** \ingroup libusb_misc
 * Capabilities supported by an instance of src on the current running
...
enum libusb_capability {
	 ...
	 * still have to call additional src functions such as
	...
	struct libusb_transfer *transfer, unsigned int packet)
{
	 ...
	 * signed to avoid compiler warnings. FIXME for src-2. */
	if (packet > INT_MAX)
		return NULL;
	_packet = (int) packet;
 ...
 *
 * src will call this function later, when a matching event had happened on
 ...
 * already plugged into the machine. Note that src modifies its internal
...
enum libusb_option {
	 ...
	 *
	 * You are advised to use level LIBUSB_LOG_LEVEL_WARNING. src is conservative
	 * with its message logging and most of the time, will only log messages that
	 * explain error conditions and other oddities. This will help you debug
	 * your software.
	 *
	 * If the LIBUSB_DEBUG environment variable was set when src was
	 * initialized, this function does nothing: the message verbosity is fixed
	 * to the value in the environment variable.
	 *
	 * If src was compiled without any message logging, this function does
	 * nothing: you'll never get any messages.
	 *
	 * If src was compiled with verbose debug message logging, this function
